                ...
                <table >
                    ...
                    <c:forEach items="${pageResult.rows}" var="list" varStatus="status">
                        <tr>
                            ...
                            <c:if test="${list.gmt_create != null}">
                                <td onclick='fillMaterialSurvey("${list.id}")'>查看</td>
                            </c:if>
...
<script>
    ...
    function querylist(pageNum,pageSize) {
        window.location.href=contextpath+'orgSurvey/tolist.action?pageNum='+pageNum+'&pageSize='+pageSize;
    }

    function query() {
        var state=$("#select-search-status").val();
        window.location.href=contextpath+'orgSurvey/tolist.action?state='+state+"&materialId="+${materialId};
    }

    function add() {
        window.location.href=contextpath+'research/toinsert.action';
    }
