...
public class WriterUserServiceImpl implements WriterUserService {
	...
	public WriterUserCertificationVO showTeacherCertification(Long userId) {
		...
		if (StringUtil.notEmpty(cert)) {
			...
			String getFileName = getFile.getFilename(); // 获取文件名
			String getFileNameEnd = getFileName.substring(getFileName.lastIndexOf(".")+1); // 获取后缀
			String getFileNameNew = "教师资格证";
			writerUserCertificationVO.setCertName(getFileNameNew + getFileNameEnd);
		}
	...
	public WriterUserCertification updateTeacherCertification(WriterUserCertification writerUserCertification, 
		...
		if (ObjectUtil.isNull(id)) { //id为空就增加否则修改
	        ...
	        if (certFile.isEmpty()) {
	            ...
	            mongoId = fileService.save(certFile, ImageType.WRITER_USER_CERT, writerUserCertifications.getId());
	            if (null != mongoId) {
